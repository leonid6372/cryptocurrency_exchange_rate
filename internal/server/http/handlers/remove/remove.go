...
func New(log *slog.Logger, cryptoExRate CryptoExRate) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		const op = "internal.server.http.handlers.remove.New"
		...
		err = cryptoExRate.RemoveCurrency(req.Coin)
		if err != nil {
			log.Error("failed to remove cryptocurrency for monitoring", sl.Err(err))
			w.WriteHeader(500)
			render.JSON(w, r, resp.Error("failed to remove cryptocurrency for monitoring"))
			return
		}
